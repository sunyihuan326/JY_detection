...
def tiny_backbone(input_data, trainable):
    with tf.variable_scope('darknet_tiny'):

        input_data = common.convolutional(input_data, filters_shape=(3, 3, 3, 16), trainable=trainable, name='conv0')
        input_data = tf.layers.max_pooling2d(input_data, 2, 2)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 16, 32), trainable=trainable, name='conv1')
        input_data = tf.layers.max_pooling2d(input_data, 2, 2)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 32, 64), trainable=trainable, name='conv2')
        input_data = tf.layers.max_pooling2d(input_data, 2, 2)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 64, 128), trainable=trainable, name='conv3')
        input_data = tf.layers.max_pooling2d(input_data, 2, 2)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 128, 256), trainable=trainable, name='conv4')
        route_1 = input_data

        input_data = tf.layers.max_pooling2d(input_data, 2, 2)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 256, 512), trainable=trainable, name='conv5')
        input_data = tf.layers.max_pooling2d(input_data, 1, 1)
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 512, 1024), trainable=trainable, name='conv6')
